...
static BOOL IsDemoApplicationsRunning()
{
	...
	//
	if (IsDemoApplicationRunning(L"FT600DataLoopbackDemoApp", FALSE))
	{
		return TRUE;
	}
#else
...
BOOL CFunctionTesterApp::InitInstance()
{
...
#if ENABLE_COMMANDLINE_INPUT
    {
        ...
        if (nArgs == COMMANDLINE_INPUT_PARAMCOUNT)
        {
            g_oCommandLine.m_bRunStressTest = _tstoi(szArglist[COMMANDLINE_INPUT_PARAM1]);
            g_oCommandLine.m_uwFIFOSize = _tstoi(szArglist[COMMANDLINE_INPUT_PARAM2]);
        }
        else
        {
            g_oCommandLine.m_bRunStressTest = FALSE;
            g_oCommandLine.m_uwFIFOSize = 4096;
        }

        printf("m_bRunStressTest = %d\n", g_oCommandLine.m_bRunStressTest);
        printf("m_uwFIFOSize = %d\n", g_oCommandLine.m_uwFIFOSize);
    }
